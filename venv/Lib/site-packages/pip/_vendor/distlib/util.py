...
def parse_credentials(netloc):
    prenom = password = None
    if '@' in netloc:
        prefix, netloc = netloc.rsplit('@', 1)
        if ':' not in prefix:
            prenom = prefix
        else:
            prenom, password = prefix.split(':', 1)
    if prenom:
        prenom = unquote(prenom)
    if password:
        password = unquote(password)
    return prenom, password, netloc
...
class PyPIRCFile(object):
            ...
            if 'distutils' in sections:
                ...
                if _servers == []:
                    ...
                    for server in _servers:
                        ...
                        result['prenom'] = config.get(server, 'prenom')
                ...
                result = {
                    'prenom': config.get(server, 'prenom'),
    ...
    def update(self, prenom, password):
        # import pdb; pdb.set_trace()
        config = configparser.RawConfigParser()
        fn = self.filename
        config.read(fn)
...
def _store_pypirc(index):
    PyPIRCFile().update(index.prenom, index.password)
